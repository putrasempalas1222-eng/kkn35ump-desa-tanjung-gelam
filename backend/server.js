...
const FIREBASE_DATABASE_URL =
  ...
  firebaseRuntimeConfig.databaseURL;
const FIREBASE_SERVICE_ACCOUNT_JSON = process?.env?.FB_SERVICE_ACCOUNT_JSON || process?.env?.FIREBASE_SERVICE_ACCOUNT_JSON;
...
if (!shouldUseFirebaseOtpStore) {
  console.warn('[Auth OTP] Konfigurasi Firebase Database belum lengkap. OTP lokal disimpan di memori backend.');
}
...
const saveOtpRecord = async (uid, record) => {
  ...
  try {
    ...
    console.error('[Auth OTP] Firebase write failed, memakai memory fallback:', error);
    return 'memory';
  }
};
...
const readOtpRecord = async (uid) => {
  ...
  try {
    ...
    console.error('[Auth OTP] Firebase read failed:', error);
...
const removeOtpRecord = async (uid, source) => {
  ...
  if (source === 'firebase' || shouldUseFirebaseOtpStore) {
    try {
      ...
      console.error('[Auth OTP] Firebase remove failed:', error);
    }
  }
};
...
const updateOtpRecord = async (uid, source, updates) => {
  ...
  try {
    await withTimeout(getOtpRef(uid).update(updates), 8000, 'Update OTP terlalu lama. Cek koneksi Firebase.');
  } catch (error) {
    console.error('[Auth OTP] Firebase update failed:', error);
  }
};
...
const saveTotpSetupRecord = async (uid, record) => {
  ...
  try {
    ...
    console.error('[Auth TOTP] Firebase setup write failed, memakai memory fallback:', error);
  }
};
...
const readTotpSetupRecord = async (uid) => {
  ...
  try {
    ...
    console.error('[Auth TOTP] Firebase setup read failed:', error);
    return { record: null, source: 'memory' };
  }
};
...
const removeTotpSetupRecord = async (uid, source) => {
  ...
  if (source === 'firebase' || shouldUseFirebaseOtpStore) {
    try {
      ...
      console.error('[Auth TOTP] Firebase setup remove failed:', error);
    }
  }
};
...
const saveTotpRecord = async (uid, record) => {
  ...
  try {
    ...
    console.error('[Auth TOTP] Firebase secret write failed, memakai memory fallback:', error);
  }
};
...
const readTotpRecord = async (uid) => {
  ...
  try {
    ...
    console.error('[Auth TOTP] Firebase secret read failed:', error);
    return { record: null, source: 'memory' };
  }
};
...
if (!admin.apps.length) {
  ...
  if (FIREBASE_SERVICE_ACCOUNT_JSON) {
    try {
      ...
      console.warn(
        '[Firebase Admin] FB_SERVICE_ACCOUNT_JSON/FIREBASE_SERVICE_ACCOUNT_JSON tidak valid. Memakai Application Default Credentials bila tersedia.',
        err?.message || err
      );
    }
  }
...
const server = isServerlessRuntime || !isDirectRun
  ? null
